...
use App\Models\APICampaignCheckPhone;
use Importer;
...
class CheckPhoneController extends Controller
{
    ...
    public function importDataCheckPhone(Request $request)
    {
    	if(Session::has("client_id"))
        {
            ...
            if($campaigns)
            {
            	return view('clients.import_check_phone', compact('data', 'campaigns', 'campaign_id', 'campaign_selected'));
            }
            else
            {
    ...
    function listDataCheckPhoneAjax(Request $requests)
    {
		...
		{
			$where[] = ['status', 'like', "%".$requests->get("status")."%"];
		}

		if($requests->get("time_call_from"))
		{
			$where[] = ['time_call', '>=', $requests->get("time_call_from")];
		}
		if($requests->get("time_call_to"))
		{
			$where[] = ['time_call', '<=', $requests->get("time_call_to")];
		}

		$iTotalRecords = DataCampaignCheckPhone::where($where)->count();
		...
		$data = DataCampaignCheckPhone::where($where)->offset($start)->limit($iDisplayLength)->orderBy($order, $by)->get();
		...
		foreach($data as $dt) {
			...
			$dt->phone,
			$dt->status,
			!empty($dt->time_call) ? date("d/m/Y H:i:s", strtotime($dt->time_call)) : '',
    ...
    public function postImportDataCheckPhone(Request $request)
    {
    	$result = new \stdClass();
        $result->status = false;
        ...
        if(Session::has("client_id"))
        {
	        ...
	        else 
	        {
				...
				$fileName  = 'tmp-phone-'.$campaign_id.'.'.$extension;
    ...
    public function reportDataPhone()
    {
    	if(Session::has("client_id"))
    	{
    		$campaigns 	= Campaign::where("member_id", Session::get("client_id"))->get();
    		return view("clients.report_check_phone", compact('campaigns'));
    	}
    	else
    	{
    		return Redirect::to("/client");
    ...
    public function reportDataPhoneAjax(Request $requests)
    {
    	$records = array();
		$records["data"] = array(); 
		...
		foreach($data as $dt) {
			...
			$dt->phone,
			$dt->status,
			!empty($dt->status) ? date("d/m/Y H:i:s", strtotime($dt->time_call)) : '',
			'',
			);
    ...
    public function reportDataPhoneDownload(Request $requests)
    {
    	$result = new \stdClass();
        $result->status = false;
        $result->message = "Vui lòng nhập đầy đủ thông tin";
        ...
        $file_tmp = $folder.'/phone-'.Session::get("client_id").'.xlsx'; 
		...
		(new FastExcel($this->usersGenerator($where, $order, $by)))->export($dir_final.$file_tmp, function ($user) {
		    ...
		    return [
		        ...
		        'Thời gian Kiểm tra' => (!empty($user->status) ? date("d/m/Y H:i:s", strtotime($user->time_call)) : ''),
		    ];
		});
		$path = "";
		$result->status = true;
    ...
    {
	    foreach (DataCampaignCheckPhone::where($where)->orderBy($order, $by)->cursor() as $user) {
	        yield $user;
	    }
	}
	...
	public function APICheckPhone(Request $request)
	{
		if(Session::has("client_id"))
        {
            ...
            if($campaigns)
            {
            	return view('clients.api_check_phone', compact('data', 'campaigns', 'campaign_id', 'campaign_selected'));
            }
            else
            {
            	return Redirect::to("/client");
	...
	public function createAPI(Request $request)
	{
        ...
        if(Session::has("client_id"))
        {
	        ...
	        {
	        	$check = APICampaignCheckPhone::where("member_id", Session::get("client_id"))->where("campaign_id", $campaign_id)->first();
	        	if(!$check)
	        	{
		        	$dt 					= new APICampaignCheckPhone;
	                $dt->member_id          = Session::get("client_id");
	                $dt->campaign_id        = $campaign_id;
	...
	public function changeAPI($id)
	{
		if(Session::has("client_id"))
		{
			$check = APICampaignCheckPhone::where("member_id", Session::get("client_id"))->where("id", intval($id))->first();
	    	if($check)
	    	{
	            ...
	            return Redirect::to("/client/api_phone");
	        }
	        else
	        {
	        	return Redirect::to("/client/api_phone");
	...
	public function APICheckPhoneAjax(Request $requests)
	{
		$records = array();
		$records["data"] = array(); 
		...
		$iTotalRecords = APICampaignCheckPhone::where($where)->count();
    	$iDisplayLength = intval($requests->get('length'));
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($requests->get('start'));
		$sEcho = intval($requests->get('draw'));
		...
		$data = APICampaignCheckPhone::where($where)->offset($start)->limit($iDisplayLength)->orderBy($order, $by)->get();
		...
		foreach($data as $dt) {
			...
			'<a href="'.url('/client/api_phone/renewkey/'.$dt->id).'" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Đổi API</a>',
			);
	...
	public function APICheckPhoneUpdateStatus(Request $request)
	{
		$result = new \stdClass();
        $result->status = false;
        $result->message = "Vui lòng nhập đầy đủ thông tin";
        ...
        if(Session::has("client_id"))
        {
	        ...
	        {
	        	$check = APICampaignCheckPhone::where("member_id", Session::get("client_id"))->where("id", $id)->first();
	        	if($check)
	        	{
		        	$check->status = $status;
		        	$check->save();

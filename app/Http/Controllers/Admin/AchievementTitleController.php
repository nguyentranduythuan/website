...
class AchievementTitleController extends BaseController
{
    public function index(Request $requests)
    {
    	parent::index($requests);
    	
    	$title = "achievement Info Page";

    	return view("admin.achievement_title", compact('title'));
    }
    ...
    function getDataAjax(Request $requests)
    {
		...
		$iTotalRecords 		= AchievementTitle::where($where)->count();
		...
		$data 				= AchievementTitle::where($where)->offset($start)->limit($iDisplayLength)->orderBy($order, $by)->get();
		...
		foreach($data as $dt) {
			...
			$dt->id,
			$dt->title,
			'<a href="'.url('cms/achievement-title/edit/'.$dt->id).'" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Edit</a> 
			<a href="javascript:onDelete('.$dt->id.', \''.url('cms/achievement-title/delete/'.$dt->id).'\');" class="btn btn-xs default btn-editable delete_record"><i class="fa fa-times"></i> Delete</a>',
			);
    ...
    public function addData(Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	{
	    		$user = new AchievementTitle;
	    		$user->title 			= $requests->get("name");
	    		$user->save();

	    		return response()->json(['status' => 'success', 'link' => url('/cms/achievement-title/lists')]);
	    	}
	    }
	    else
	    {
	    	$title = "About Page";
	    	return view("admin.achievement_title_add", compact('title'));
    	}
    }
    ...
    function editData($id, Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	{
	    		$user = AchievementTitle::where("id", $id)->first();
	    		if($user)
	    		{
		    		$user->title 			= $requests->get("name");
		    		$user->save();
	    ...
	    else
	    {
	    	...
	    	$content 	= AchievementTitle::where("id", $id)->first();
	    	
	    	return view("admin.achievement_title_edit", compact('title', 'content'));
    	}
    }
    ...
    function deleteData($id)
    {
		...
		return Redirect::to('cms/achievement-title/lists');
    }
}

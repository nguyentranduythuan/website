...
class AboutTitleController extends BaseController
{
    public function index(Request $requests)
    {
    	...
    	$title = "About Info Page";

    	return view("admin.about_title", compact('title'));
    }
    ...
    function getDataAjax(Request $requests)
    {
		...
		$iTotalRecords 		= AboutInfo::where($where)->count();
		...
		$data 				= AboutInfo::where($where)->offset($start)->limit($iDisplayLength)->orderBy($order, $by)->get();
		...
		foreach($data as $dt) {
			...
			$dt->id,
			$dt->title,
			'<a href="'.url('cms/about-title/edit/'.$dt->id).'" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Edit</a> 
			<a href="javascript:onDelete('.$dt->id.', \''.url('cms/about-title/delete/'.$dt->id).'\');" class="btn btn-xs default btn-editable delete_record"><i class="fa fa-times"></i> Delete</a>',
			);
    ...
    public function addData(Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	else
	    	{
	    		...
	    		return response()->json(['status' => 'success', 'link' => url('/cms/about-title/lists')]);
	    	}
	    }
	    else
	    {
	    	$title = "About Page";
	    	return view("admin.about_title_add", compact('title'));
    	}
    }
    ...
    function editData($id, Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	{
	    		$user = AboutInfo::where("id", $id)->first();
	    		if($user)
	    		{
		    		$user->title 			= $requests->get("name");
		    		$user->save();
	    ...
	    else
	    {
	    	...
	    	$content 	= AboutInfo::where("id", $id)->first();
	    	
	    	return view("admin.about_title_edit", compact('title', 'content'));
    	}
    }
    ...
    function deleteData($id)
    {
		...
		return Redirect::to('cms/about-title/lists');
    }
}

...
class BenefitContentController extends BaseController
{
    public function index(Request $requests)
    {
    	...
    	return view("admin.benefit_content", compact('title'));
    }
    ...
    function getDataAjax(Request $requests)
    {
		...
		$iTotalRecords 	= BenefitContent::where($where)->count();
		...
		$data 			= BenefitContent::where($where)->offset($start)->limit($iDisplayLength)->orderBy($order, $by)->get();
		...
		foreach($data as $dt) {
			...
			$dt->id,
			(!empty($dt->image) ? "<img width='100' src='".\URL::asset(\Config::get('constants.LINK_BENEFIT')).'/'.$dt->image."'/>" : ''),
			$dt->content,
			//$dt->title,
			...
			'<a href="'.url('cms/benefit-content/edit/'.$dt->id).'" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Edit</a> 
			<a href="javascript:onDelete('.$dt->id.', \''.url('cms/benefit-content/delete/'.$dt->id).'\');" class="btn btn-xs default btn-editable delete_record"><i class="fa fa-times"></i> Delete</a>',
			);
    ...
    public function addData(Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	{
	    		$item = new BenefitContent;
	    		$item->content 			= $requests->get("content");
	    		$item->benefit_title_id   = 1;
	    		$item->save();

	    		$tmp                = createdFolderBenefitContent();
	    		...
	    		return response()->json(['status' => 'success', 'link' => url('/cms/benefit-content/lists')]);
	    	}
	    }
	    else
	    {
	    	$title = "About Page";
	    	return view("admin.benefit_content_add", compact('title'));
    	}
    }
    ...
    function editData($id, Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	{
	    		$item = BenefitContent::where("id", $id)->first();
	    		if($item)
	    		{
		    		$item->content 			= $requests->get("content");
		    		$item->benefit_title_id   = 1;
	    ...
	    else
	    {
	    	$title = "Benefit Page";
	    	//$blogs = Blog::all();
	    	$content 		= BenefitContent::where("id", $id)->first();

	    	return view("admin.benefit_content_edit", compact('title', 'content',));
    	}
    }
    ...
    function deleteData($id)
    {
		...
		return Redirect::to('cms/benefit-content/lists');
    }
}

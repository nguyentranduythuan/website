...
class ClientTitleController extends BaseController
{
    public function index(Request $requests)
    {
    	...
    	return view("admin.client_title", compact('title'));
    }
    ...
    function getDataAjax(Request $requests)
    {
		...
		$iTotalRecords 		= ClientTitle::where($where)->count();
		...
		$data 				= ClientTitle::where($where)->offset($start)->limit($iDisplayLength)->orderBy($order, $by)->get();
		...
		foreach($data as $dt) {
			...
			$dt->id,
			$dt->title,
			'<a href="'.url('cms/client-title/edit/'.$dt->id).'" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Edit</a> 
			<a href="javascript:onDelete('.$dt->id.', \''.url('cms/client-title/delete/'.$dt->id).'\');" class="btn btn-xs default btn-editable delete_record"><i class="fa fa-times"></i> Delete</a>',
			);
    ...
    public function addData(Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	{
	    		$user = new ClientTitle;
	    		$user->title 			= $requests->get("name");
	    		$user->save();

	    		return response()->json(['status' => 'success', 'link' => url('/cms/client-title/lists')]);
	    	}
	    }
	    else
	    {
	    	$title = "About Page";
	    	return view("admin.client_title_add", compact('title'));
    	}
    }
    ...
    function editData($id, Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	{
	    		$user = ClientTitle::where("id", $id)->first();
	    		if($user)
	    		{
		    		$user->title 			= $requests->get("name");
		    		$user->save();
	    ...
	    else
	    {
	    	...
	    	$content 	= ClientTitle::where("id", $id)->first();
	    	
	    	return view("admin.client_title_edit", compact('title', 'content'));
    	}
    }
    ...
    function deleteData($id)
    {
		...
		return Redirect::to('cms/client-title/lists');
    }
}

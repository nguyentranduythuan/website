...
class About1Controller extends BaseController
{
    public function index(Request $requests)
    {
    	...
    	return view("admin.about1", compact('title'));
    }
    ...
    function getDataAjax(Request $requests)
    {
		...
		$iTotalRecords 	= About1::where($where)->count();
		...
		$data 			= About1::where($where)->offset($start)->limit($iDisplayLength)->orderBy($order, $by)->get();
		//dd($data);
		$i = 0;
		...
		foreach($data as $dt) {
			...
			'<a href="'.url('cms/about-us-1/edit/'.$dt->id).'" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Edit</a> 
			<a href="javascript:onDelete('.$dt->id.', \''.url('cms/about-us-1/delete/'.$dt->id).'\');" class="btn btn-xs default btn-editable delete_record"><i class="fa fa-times"></i> Delete</a>',
			);
    ...
    public function addData(Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	else
	    	{
	    		...
	    		return response()->json(['status' => 'success', 'link' => url('/cms/about-us-1/lists')]);
	    	}
	    }
	    else
	    {
	    	...
	    	return view("admin.about1_add", compact('title'));
    	}
    }

...
class AboutContentController extends BaseController
{
    public function index(Request $requests)
    {
    	...
    	return view("admin.about_content", compact('title'));
    }
    ...
    function getDataAjax(Request $requests)
    {
		...
		$iTotalRecords 	= AboutContent::where($where)->count();
		...
		$data 			= AboutContent::where($where)->offset($start)->limit($iDisplayLength)->orderBy($order, $by)->get();
		...
		foreach($data as $dt) {
			...
			$dt->id,
			$dt->name,
			(!empty($dt->image) ? "<img width='100' src='".\URL::asset(\Config::get('constants.LINK_ABOUTCONTENT')).'/'.$dt->image."'/>" : ''),
			//$dt->title,
			...
			'<a href="'.url('cms/about-content/edit/'.$dt->id).'" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Edit</a> 
			<a href="javascript:onDelete('.$dt->id.', \''.url('cms/about-content/delete/'.$dt->id).'\');" class="btn btn-xs default btn-editable delete_record"><i class="fa fa-times"></i> Delete</a>',
			);
    ...
    public function addData(Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	else
	    	{
	    		...
	    		return response()->json(['status' => 'success', 'link' => url('/cms/about-content/lists')]);
	    	}
	    }
	    else
	    {
	    	$title = "About Page";
	    	$category = AboutContent::all();
	    	return view("admin.about_content_add", compact('title','category'));
    	}
    }
    ...
    function editData($id, Request $requests)
    {
    	...
    	if ($requests->isMethod('post'))
    	{
	    	...
	    	{
	    		$item = AboutContent::where("id", $id)->first();
	    		if($item)
	    		{
		    		$item->name 			= $requests->get("name");
		    		$item->about_title_id   = 1;
	    ...
	    else
	    {
	    	$title = "About Page";
	    	//$blogs = Blog::all();
	    	$content 		= AboutContent::where("id", $id)->first();

	    	return view("admin.about_content_edit", compact('title', 'content',));
    	}
    }
    ...
    function deleteData($id)
    {
		...
		return Redirect::to('cms/about-content/lists');
    }
}

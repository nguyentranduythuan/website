                                ...
                                <div class="tab-pane active" id="tab_1_1">
                                    <form class="form-without-legend" role="form" id="form-create-campaign" method="post" accept="#">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" placeholder="Tên chiến dịch" {{ $campaign->is_active == 0 ? '' : 'readonly' }} name="campaign_name" value="{{ $campaign->campaign_name }}" class="form-control"/>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label">Thời gian chạy chiến dịch</label>
                                            <div class="input-group" id="defaultrange">
                                                <input type="text" class="form-control" {{ $campaign->is_active == 0 ? '' : 'readonly' }} value="{{ $campaign->start_run }} - {{ $campaign->end_run }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" placeholder="Số tiền giới hạn" {{ $campaign->is_active == 0 ? '' : 'readonly' }} name="budget_limit" value="{{ $campaign->budget_limit }}" class="form-control"/>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label">Số tiền đã sử dụng</label>
                                            <input type="number" placeholder="Số tiền giới hạn" readonly name="amount_used" value="{{ $campaign->amount_used }}" class="form-control"/>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label">Ghi chú</label>
                                            <textarea class="form-control" name="note" {{ $campaign->is_active == 0 ? '' : 'readonly' }} rows="3" placeholder="Ghi chú">{{ $campaign->note }}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label">Giá SMS</label>
                                            <input type="number" placeholder="Giá SMS" readonly="" name="price_sms" value="{{ $campaign->price_sms }}" class="form-control"/>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label">Giá Check phone</label>
                                            <input type="number" placeholder="Giá Check phone" readonly="" name="price_call" value="{{ $campaign->price_call }}" class="form-control"/>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            $status = $status_list[$campaign->is_active];
                                            ?>
                                            <input type="text" placeholder="Giá Check phone" readonly="" name="price_call" value="{{ current($status) }}" class="form-control"/>
                                        </div>
                                        @if($campaign->is_active == 0)
                                        <div class="margiv-top-10">
                                            <button type="submit" id="btn_create_campagin" class="btn green-haze uppercase">Edit</button>
                                        </div>
                                        @endif
                                    </form>
                                </div>
